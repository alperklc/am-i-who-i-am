function loadStuff()
	...
	introSnd = love.audio.newSource("snd/menu.mp3", "static")
	click = love.audio.newSource("snd/click.mp3", "static")

	gmThreeSnd = love.audio.newSource("snd/gmThree.mp3", "static")
	gmThreeSnd:setVolume(0.2)

	gmSnd = love.audio.newSource("snd/gmSnd.mp3", "static")
	gmSnd:setVolume(0.2)
...
function fileWork()
	...
	if not love.filesystem.getInfo('sys.oge') then
		file:open('w')
		file:write("x")
		file:close()
	else

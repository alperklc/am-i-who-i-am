 function updateGameOne(st, dt)
	...
	elseif st == 1 then
		...
		if  timeCounter > 50  then
			...
			timeCounter = timeCounter + dt * 100
		end	
 ...
 function drawGameOne(st)
	if st == 1 then
		...
		if love.keyboard.isDown("a") then
			...
			d = false
			charAnim:draw(hero.x, hero.y, 0, -1, 1, 30)
		elseif love.keyboard.isDown("d") then
			...
			dir = 0
			if d == true  then
				love.graphics.draw(heroStable, hero.x, hero.y, 0, 1, 1, 25)
			else
				love.graphics.draw(heroStable, hero.x, hero.y, 0, -1, 1, 25)
			end
		end
